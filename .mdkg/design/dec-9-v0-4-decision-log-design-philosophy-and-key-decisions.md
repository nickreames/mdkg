...
id: dec-9
type: dec
title: v0.4 decision log design philosophy and key decisions
status: accepted
...
This decision record is the v0.4 rollup of mdkg design philosophy and high-impact architecture decisions. It preserves the "why" so implementation work remains aligned with deterministic, local-first constraints.

The rollout baseline is still v0.0.3 source behavior. Any v0.4 target-state decisions that differ from current runtime are explicitly marked as planned.
...
Current-gap note:
- v0.0.3 currently includes pinned core docs through `pack --verbose`; v0.4 target-state treats constrained core context inclusion as a design default policy.
...
## DEC-04 No vector DB in v0.4 core

Context: vector retrieval adds probabilistic behavior and operational overhead.

Decision: v0.4 retrieval remains path/ID/graph/metadata driven.
...
Decision: v0.4 plans `init --omni` scaffolding with SOUL/HUMAN, skills scaffold, and core pin updates.
...
Context: multi-file profile sprawl is high-friction and drift-prone at v0.4 scope.

Decision:
- canonical HUMAN path is `.mdkg/core/HUMAN.md`
- HUMAN is a strict mdkg node (not a twin directory contract in v0.4)
...
- avoid markdown files under `.mdkg/work/events/` in v0.4 docs because they are parsed as strict nodes
...
Context: v0.4 planning had unresolved interface and policy questions around skills command shape, stage access control, and latest-checkpoint selection.

Decision:
- keep skills capabilities in existing mdkg command families (no new `mdkg skills` namespace in v0.4 docs)
...
- Treat v0.4 as implementation-only with no philosophy rollup (rejected): future changes lose rationale and regress toward non-deterministic patterns.
- Adopt vector-first retrieval now (rejected): violates determinism-first v0.4 scope.

# Consequences

- v0.4 implementation planning now has a concise decision baseline to validate against.
