...
id: edd-7
type: edd
title: 0.0.4 agent skills standards alignment and research snapshot
...
This document captures mdkg 0.0.4 alignment to external Agent Skills best-practice guidance.
...
Deferred or constrained in 0.0.4:
...
- generic `list/show/search` skill access has been removed in favor of a single `mdkg skill ...` discovery surface before publish
...
Stage-aware gating model in 0.0.4 docs:
...
Baseline `SKILL.md` fields adopted for 0.0.4 docs:
...
- use `mdkg skill list/show/search` as the only supported skill discovery UX
- provide `mdkg skill new` and `mdkg skill validate` as first-class authoring commands
- keep tag-aware discovery filters (`--tags`, `--tags-mode any|all`) on the skill-focused query surfaces
- add `--json` for machine-readable skill discovery in the first structured-output wave
...
Refresh cadence:
- review this snapshot at each `0.0.4.x` release cut
- refresh again before the next release-planning lock
...
1. integrate this research snapshot into 0.0.4 design graph
2. align skill query/gating tasks/tests to this contract
3. refresh snapshot as part of each 0.0.4.x docs alignment audit
4. keep the `mdkg skill` scaffold and docs aligned to the source-truth dogfood skills for later-release work
