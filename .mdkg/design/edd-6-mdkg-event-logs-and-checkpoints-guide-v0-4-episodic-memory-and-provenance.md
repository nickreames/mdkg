...
id: edd-6
type: edd
title: mdkg event logs and checkpoints guide v0.4 episodic memory and provenance
...
This document defines mdkg v0.4 episodic-memory guidance: how to capture execution provenance over time without bloating semantic graph context or spamming git history.
...
v0.4 episodic memory uses a two-tier model:
...
- no markdown logs inside `.mdkg/work/events/` in v0.4 docs due strict-node parsing
...
Capability contracts for v0.4 docs:
...
- Validation/reporting surfaces malformed JSONL now; redaction-policy failures remain documentation-level in v0.4.
...
Doc tracking:
- this document is Doc 8 (`edd-6`) and completes the mdkg v0.4 docs set in planning form
