...
id: edd-9
type: edd
title: 0.0.4x product simplification and onboarding for humans and agents
...
This document defines the post-0.0.4 product simplification direction for mdkg so the tool is easier to adopt for two primary audiences:
...
- `mdkg skill list` now emits explicit count and empty-state output, and `--json` is the machine-readable discovery surface.
