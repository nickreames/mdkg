...
id: edd-4
type: edd
title: mdkg init omni specification v0.4
...
This document defines the v0.4 `mdkg init --omni` contract as an opinionated, minimal bootstrap that makes a repo agent-ready while preserving mdkg determinism and source-truth policy.
...
Non-goals:
- no `.mdkg/twin/` contract in v0.4 (canonical HUMAN is under `.mdkg/core/`)
...
Deferred/non-goal for v0.4:
- no new example-skill toggle is required in v0.4 contract docs.
...
- may be created by implementation, but is not a required default artifact in v0.4 docs
