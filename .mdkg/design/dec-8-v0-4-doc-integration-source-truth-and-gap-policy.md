...
id: dec-8
type: dec
title: v0.4 doc integration source truth and gap policy
status: accepted
...
We are integrating forward-looking v0.4 architecture and product docs into the mdkg graph while current source implementation remains v0.0.3-era behavior. We need a stable policy for documenting planned behavior without misrepresenting implemented behavior.

# Decision

- Source code is the truth source for current behavior.
- v0.4 content is documented as planned target state with explicit gap notes.
...
- Event log format is locked to `.mdkg/work/events/*.jsonl` for v0.4 planning.
...
- Last-checkpoint inclusion in packs is a v0.4 default requirement when a checkpoint exists.
...
- External orchestrator interoperability in v0.4 docs uses a minimal structured run/event/artifact contract.
...
- Manual docs alignment audits are the v0.4 governance path in this phase (no scripted parity checks).
- No physical scaffolding changes are made in this pass for planned v0.4 paths.
- Episodic logging behavior is documented now and implemented later in runtime work.
- v0.4 decision log updates are amended in place under this decision record.
...
- Current-state-only docs (rejected): loses v0.4 planning clarity and roadmap utility.
- Freeform `SOUL.md`/`HUMAN.md` outside strict schema (rejected): conflicts with deterministic indexing model.

# Consequences

- mdkg docs can evolve toward v0.4 without violating source-truth discipline.
