...
id: dec-10
type: dec
title: v0.4 command freeze stage gating and checkpoint selection
status: accepted
...
v0.4 gap analysis identified unresolved contract decisions that were blocking implementation-quality planning: command surface shape for skills, stage-gated skill discovery behavior, checkpoint selection semantics, redaction policy framing, and orchestrator interoperability boundaries.

# Decision

- Skills capabilities in v0.4 are documented as extensions to existing command families (`list`, `show`, `search`, `pack`, `validate`, `init`), not a new `mdkg skills` namespace.
...
- Redaction remains a v0.4 documentation policy contract (implementation deferred), with policy-level `safe` and `strict` framing.
- External orchestrator integration adopts a minimal structured contract for run/event/artifact exchange.
- Documentation alignment governance for v0.4 uses manual audits, not scripted parity enforcement.
...
- Define full runtime protocol spec now (rejected): over-scoped for v0.4 docs-only pass.

# Consequences

- v0.4 planning is decision-complete for command and orchestration direction without runtime mutation in this pass.
