...
Common public-alpha mdkg setup and validation issues.
...
In a fresh repo, `node: null` can simply mean no work exists yet. Continue with [If no work exists yet](quickstart.md#if-no-work-exists-yet) and create a small task before expecting `goal next` to route.
