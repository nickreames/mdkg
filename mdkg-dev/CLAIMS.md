...
| Home | `0.4.0` launch-track copy is pre-publish web language, while package metadata remains tied to the current published npm package | `package.json`, npm registry, mdkg.dev JSON-LD smoke, goal-42 evidence | source-only until deploy | Say `mdkg@0.3.9` is the published package baseline until a real `0.4.0` publish happens; local validation is pre-publish, live validation is post-publish/deploy. | mdkg maintainer | approved for local validation |
