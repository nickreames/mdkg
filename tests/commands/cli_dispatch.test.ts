...
test("cli returns usage errors for unknown and malformed commands", () => {
  ...
  assert.match(badDb.stderr, /mdkg db requires index\/init\/migrate\/verify\/stats\/queue\/snapshot/);
  ...
  const dbVerify = runCli(root, ["db", "verify", "--json"]);
  assert.equal(dbVerify.status, 0, dbVerify.stderr);
  assert.equal(JSON.parse(dbVerify.stdout).action, "db-verify");
  ...
  assert.match(badDbSnapshot.stdout, /mdkg db snapshot seal \[--queue-policy drain\|paused\] \[--json\]/);
