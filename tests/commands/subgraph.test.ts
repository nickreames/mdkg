...
test("subgraph audit and upgrade-plan expose read-only downstream safety receipts", () => {
  const root = makeTempDir("mdkg-subgraph-audit-plan-");
  run(["init", "--agent"], root);
  const child = createGitChildBundle(root, "child_audit");
  run([
    "subgraph",
    "add",
    "child_audit",
    child.bundlePath,
    "--source-path",
    "projects/child_audit",
    "--json",
  ], root);
  const beforeBundleHash = sha256File(child.bundleAbs);
  const beforeConfig = fs.readFileSync(path.join(root, ".mdkg", "config.json"), "utf8");
  ...
  const audit = json<{
    ...
    subgraphs: Array<{
      ...
      capability_summary: { spec_count: number; work_count: number; node_count: number };
      checks: Array<{ id: string; ok: boolean }>;
    }>;
  }>(run(["subgraph", "audit", "child_audit", "--target", ".mdkg/subgraphs", "--json"], root).stdout);
  assert.equal(audit.action, "audited");
  assert.equal(audit.ok, true);
  assert.equal(audit.subgraphs[0].alias, "child_audit");
  assert.equal(audit.subgraphs[0].dirty_tracked, false);
  assert.equal(audit.subgraphs[0].capability_summary.spec_count, 1);
  assert.equal(audit.subgraphs[0].capability_summary.work_count, 1);
  assert.ok(audit.subgraphs[0].capability_summary.node_count >= 1);
  assert.match(audit.subgraphs[0].source_repo_current ?? "", new RegExp(`${child.head}$`));
  assert.equal(audit.subgraphs[0].checks.some((check) => check.id === "subgraph.bundle.root_owned" && check.ok), true);
  assert.equal(audit.subgraphs[0].checks.some((check) => check.id === "subgraph.materialize.target_safe" && check.ok), true);
  ...
  const plan = json<{
    ...
    subgraphs: Array<{
      alias: string;
      capability_summary: { spec_count: number; work_count: number };
      actions: Array<{ action: string; status: string; command?: string }>;
    }>;
  }>(run(["subgraph", "upgrade-plan", "child_audit", "--json"], root).stdout);
  assert.equal(plan.action, "upgrade_plan");
  assert.equal(plan.ok, true);
  assert.equal(plan.apply_supported, false);
  assert.equal(plan.mutation_policy, "read_only_plan");
  assert.equal(plan.subgraphs[0].capability_summary.spec_count, 1);
  assert.equal(plan.subgraphs[0].capability_summary.work_count, 1);
  assert.equal(plan.subgraphs[0].actions.some((action) => action.action === "subgraph.sync" && action.status === "planned"), true);
  ...
  const outputDir = path.join(root, ".mdkg", "subgraphs", "child_audit");
  fs.mkdirSync(outputDir, { recursive: true });
  fs.writeFileSync(path.join(outputDir, "note.txt"), "not generated by mdkg\n", "utf8");
  const unsafeAudit = runFailure(["subgraph", "audit", "child_audit", "--target", ".mdkg/subgraphs", "--json"], root);
  assert.equal(unsafeAudit.status, 2);
  const unsafeReceipt = json<{ ok: boolean; errors: string[] }>(unsafeAudit.stdout);
  assert.equal(unsafeReceipt.ok, false);
  assert.equal(unsafeReceipt.errors.some((error) => error.includes("subgraph.materialize.target_safe")), true);
});

test("subgraph upgrade-plan blocks dirty child repos without mutating state", () => {
  const root = makeTempDir("mdkg-subgraph-upgrade-dirty-");
  run(["init", "--agent"], root);
  const child = createGitChildBundle(root, "child_plan_dirty");
  run([
    "subgraph",
    "add",
    "child_plan_dirty",
  ...
  const audit = json<{ ok: boolean; warnings: string[]; subgraphs: Array<{ dirty_tracked?: boolean }> }>(
    run(["subgraph", "audit", "child_plan_dirty", "--json"], root).stdout
  );
  assert.equal(audit.ok, true);
